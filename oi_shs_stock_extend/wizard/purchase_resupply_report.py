...
class ResupplyReport(models.TransientModel):
 ...
 def fuel_report(self):
    ...
    # Fetch purchase orders and process
            ...
            if subcontracts:
                ...
                for pol in po.order_line: 
                    ...
                    for val in pick:
                        ...
                        # Retrieve and write the receipt date
                        receipt_date = val.picking_id.date_done
                        worksheet.write(row, col, str(receipt_date.strftime('%d/%m/%Y') if receipt_date else 'N/A'), style_normal)
                        col += 1

                        # Determine the state of the receipt
                        state = {
                            'draft': 'Draft',
                            ...
                            if sub.origin == val.picking_id.name:
                                ...
                                for sl in at_line:
                                    if sl.picking_id.name not in supply:
                                        ...
                                        # Determine the state of the supply
                                        state = {
                                            'draft': 'Draft',
                                            'waiting': 'Waiting for another Operation',
                                            'confirmed': 'Waiting',

...
class BOMStructureXl(models.TransientModel):
   ...
   def action_generate_xls(self):
        ...
        for rec in bom_ids:
            ...
            # Process components of the BOM to calculate total cost (excluding parent)
            for a in vals['docs']:
                currency = a['currency']
                for line in a['lines']:
                    if line['type'] == 'bom' or line['type'] == 'component':
                        if line['type'] != 'subcontract':
                            ...
                            # Add component cost to total BOM cost (excluding parent)
                            if not is_main_component:  # Exclude main component (parent)
                            ...
                            # Add component data to rows (excluding parent)
                            rows.append((
            ...
            # Adding parent BOM cost and total BOM cost (excluding parent product's BOM cost in the total)
            rows.insert(0, (
                ...
                currency.symbol + str("%.2f" % round(total_bom_cost, 2)),  # Total BOM cost (components only)
            ))
